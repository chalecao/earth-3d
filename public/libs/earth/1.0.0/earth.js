...
//开启标准模式，以后的所有js代码需要符合标准规范
	...
	var REMAINING_HALF = "▪▪▪▪▪▪▪▪▪▪▪▪▫▫▫▫▫▫▫▫▫";   // glyphs for remaining progress bar and half progress
	...
	// holds the page's current configuration settings
	var configuration =
		µ.buildConfiguration(globes, products.overlayTypes);
	// interprets drag/zoom operations
	 ...
	 * D3内置的行为用于绑定当前document的drag/zoom事件，输入控制器则将D3的事件解释成globe的移动操作
	 ...
	 * D3的移动事件的先后顺序是 "zoomstart" -> ("zoom")* -> "zoomend"这个事件链。在 “zoom”事件过程中，
	 * scale和鼠标都会改变，意味着会有缩放和拖动的操作。而这些操作显然是噪点。一个平滑的缩放操作应该是一系列的
	 * "zoomstart" ->* "zoom" -> "zoomend"事件链的组合。去抖（debouncer）器是通过添加一段时间延时来消除这种噪声
	 ...
	 * 一个简单的点击事件链是这样的："zoomstart" -> "zoomend"。还有一些极端的例子，比如一种只移动globe几个像素的
	 * 伪拖动。
	 ...
	 * 该对象抛出干净的事件，对于移动操作，抛出："moveStart" -> ("move")* -> "moveEnd"事件。对于常规的点击则抛出
	 * "click"事件。伪移动则不抛出任何事件。
	 */
	function buildInputController() {
		...
		// d3.behavior.zoom - create a zoom behavior.Constructs a new zoom behavior. Once constructed,
...
// 绘制网格
	 ...
	 * 注册所有的事件监听器来把组件和页面元素绑定起来
	 */
	function init() {
		//显示状态
		report.status("Initializing...");

		//关于https://www.facebook.com/EarthWindMap
		...
		//fill-screen是绘制的渲染svg图层，包括graticule标线方格图，animation canvas是用于绘制图层的，
		// 其他的用于绘制地球上的形状
		...
		//fieldAgent =  the interpolated wind vector field
		...
		// animatorAgent = the wind animator
		...
		// overlayAgent = color overlay over the animation
